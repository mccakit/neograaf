...
inline vertex_set_t do_get_intersection(const vertex_set_t& lhs,
                                        const vertex_set_t& rhs) {
  vertex_set_t result{};
  for (const auto& v : lhs) {
    if (rhs.contains(v)) result.insert(v);
  }
  return result;
}

inline vertex_set_t set_union(const vertex_set_t& lhs,
                              const vertex_set_t& rhs) {
...
inline vertex_set_t set_difference(const vertex_set_t& lhs,
                                   const vertex_set_t& rhs) {
  vertex_set_t result = lhs;
  for (const auto& v : rhs) result.erase(v);
  return result;
}
...
void do_bron_kerbosch_maximal_clique(
  ...
  for (const auto& vertex : to_process) {
    ...
    auto excluded_intersection  = do_get_intersection(excluded_vertices, neighbors);
...
template <typename V, typename E>
[[nodiscard]] std::vector<std::vector<vertex_id_t>> bron_kerbosch(
    const graph<V, E, graph_type::UNDIRECTED>& graph) {
